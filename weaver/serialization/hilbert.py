...
if __name__=='__main__':
    # test hilbert encode and decode
    n = 2  # number of points
    dim = 224  # dimensions of the image
    num_bits = 16
    # Generate random points in a 32x32 grid
    # points = torch.randint(0, dim, (n, 2))
    points = torch.tensor([[32, 32], [0, 0]])
    print(f'what are original points: {points}')
    hilbert_code = encode(locs=points, num_dims=2, num_bits=num_bits)
    print(f'what is the encoded hilbert integer: {hilbert_code}')
    points_decoded = decode(hilbert_code, num_dims=2, num_bits=num_bits)
    print(f'what are decoded points: {points_decoded}')
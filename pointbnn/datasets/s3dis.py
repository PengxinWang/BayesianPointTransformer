...
import os
from .defaults import DefaultDataset, DynamicDataset
...
@DATASETS.register_module()
class DynamicS3DISDataset(DynamicDataset):
    def get_data_name(self, idx):
        remain, room_name = os.path.split(self.dataset.data_list[idx % len(self.data_list)])
        remain, area_name = os.path.split(remain)
        return f"{area_name}-{room_name}"